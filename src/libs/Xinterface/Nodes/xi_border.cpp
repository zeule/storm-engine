...
{
    m_bUse = false;
    BGFX_PICTURE_TEXTURE_RELEASE(pPictureService, m_sGroupName.c_str(), m_idTex);
...
void CXI_BORDER::LoadIni(INIFILE *ini1, const char *name1, INIFILE *ini2, const char *name2)
{
    ...
    {
        m_sGroupName = param;
        m_idTex = pPictureService->BGFXGetTextureID(m_sGroupName.c_str());
    }
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "lefttop_pic", param, sizeof(param), ""))
        m_nLeftTopPicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    if (m_nLeftTopPicture < 0)
    {
        ...
        pPictureService->BGFXGetTexturePos(m_nLeftTopPicture, m_frLeftTopUV);
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "righttop_pic", param, sizeof(param), ""))
        m_nRightTopPicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    if (m_nRightTopPicture < 0)
    {
        ...
        pPictureService->BGFXGetTexturePos(m_nRightTopPicture, m_frRightTopUV);
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "leftbottom_pic", param, sizeof(param), ""))
        m_nLeftBottomPicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    if (m_nLeftBottomPicture < 0)
    {
        ...
        pPictureService->BGFXGetTexturePos(m_nLeftBottomPicture, m_frLeftBottomUV);
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "rightbottom_pic", param, sizeof(param), ""))
        m_nRightBottomPicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    if (m_nRightBottomPicture < 0)
    {
        ...
        pPictureService->BGFXGetTexturePos(m_nRightBottomPicture, m_frRightBottomUV);
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "topline_pic", param, sizeof(param), ""))
        m_nTopLinePicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    if (m_nTopLinePicture < 0)
    {
        ...
        pPictureService->BGFXGetTexturePos(m_nTopLinePicture, m_frTopLineUV);
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "bottomline_pic", param, sizeof(param), ""))
        m_nBottomLinePicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    if (m_nBottomLinePicture < 0)
    {
        ...
        pPictureService->BGFXGetTexturePos(m_nBottomLinePicture, m_frBottomLineUV);
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "leftline_pic", param, sizeof(param), ""))
        m_nLeftLinePicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "rightline_pic", param, sizeof(param), ""))
        m_nRightLinePicture = pPictureService->BGFXGetImageNum(m_sGroupName.c_str(), param);
    if (m_nRightLinePicture < 0)
    {
        ...
        pPictureService->BGFXGetTexturePos(m_nRightLinePicture, m_frRightLineUV);
...
void CXI_BORDER::FillVertexBuffers()
{
    ...
    //@BGFX TODO implement batched vertex submission
    /*
    ...
    */
    ...
    // top line
    WriteVertexForSquare(0, m_frTopLineUV, m_dwColor, m_rect.left + m_pntLeftTopSize.x, m_rect.top,
                         m_rect.right - m_pntRightTopSize.x, m_rect.top + m_nTopLineHeight);
    // bottom line
    WriteVertexForSquare(4, m_frBottomLineUV, m_dwColor, m_rect.left + m_pntLeftTopSize.x,
                         m_rect.bottom - m_nBottomLineHeight, m_rect.right - m_pntRightTopSize.x, m_rect.bottom);
    // left line
    WriteVertexForSquare(8, m_frLeftLineUV, m_dwColor, m_rect.left, m_rect.top + m_pntLeftTopSize.y,
                         m_rect.left + m_nLeftLineWidth, m_rect.bottom - m_pntLeftBottomSize.y);
    // right line
    WriteVertexForSquare(12, m_frRightLineUV, m_dwColor, m_rect.right - m_nRightLineWidth,
                         m_rect.top + m_pntRightTopSize.y, m_rect.right, m_rect.bottom - m_pntRightBottomSize.y);
    // left top corner
    WriteVertexForSquare(16, m_frLeftTopUV, m_dwColor, m_rect.left, m_rect.top, m_rect.left + m_pntLeftTopSize.x,
                         m_rect.top + m_pntLeftTopSize.y);
    // right top corner
    WriteVertexForSquare(20, m_frRightTopUV, m_dwColor, m_rect.right - m_pntRightTopSize.x, m_rect.top,
                         m_rect.right, m_rect.top + m_pntRightTopSize.y);
    // left bottom corner
    WriteVertexForSquare(24, m_frLeftBottomUV, m_dwColor, m_rect.left, m_rect.bottom - m_pntLeftBottomSize.y,
                         m_rect.left + m_pntLeftBottomSize.x, m_rect.bottom);
    // right bottom corner
    WriteVertexForSquare(28, m_frRightBottomUV, m_dwColor, m_rect.right - m_pntRightBottomSize.x,
                         m_rect.bottom - m_pntRightBottomSize.y, m_rect.right, m_rect.bottom);
    ...
    if (m_nCaptionHeight > 0 && m_pCaptionImage)
    {
        ...
        // caption line
        WriteVertexForSquare(32, m_frTopLineUV, m_dwColor, m_rect.left + m_nLeftLineWidth,
                             m_rect.top + m_nCaptionHeight, m_rect.right - m_nRightLineWidth,
                             m_rect.top + m_nCaptionHeight + m_mCaptionDividerHeight);
    }
...
void CXI_BORDER::WriteVertexForSquare(uint32_t index, FXYRECT &UVRect, uint32_t dwColor, long left, long top,
                                      long right, long bottom)
{
    pV[index + 0].color = dwColor;
    pV[index + 0].pos.x = static_cast<float>(left);
    pV[index + 0].pos.y = static_cast<float>(top);
    pV[index + 0].pos.z = 1.f;
    pV[index + 0].tu = UVRect.left;
    pV[index + 0].tv = UVRect.top;

    pV[index + 1].color = dwColor;
    pV[index + 1].pos.x = static_cast<float>(left);
    pV[index + 1].pos.y = static_cast<float>(bottom);
    pV[index + 1].pos.z = 1.f;
    pV[index + 1].tu = UVRect.left;
    pV[index + 1].tv = UVRect.bottom;

    pV[index + 2].color = dwColor;
    pV[index + 2].pos.x = static_cast<float>(right);
    pV[index + 2].pos.y = static_cast<float>(top);
    pV[index + 2].pos.z = 1.f;
    pV[index + 2].tu = UVRect.right;
    pV[index + 2].tv = UVRect.top;

    pV[index + 3].color = dwColor;
    pV[index + 3].pos.x = static_cast<float>(right);
    pV[index + 3].pos.y = static_cast<float>(bottom);
    pV[index + 3].pos.z = 1.f;
    pV[index + 3].tu = UVRect.right;
    pV[index + 3].tv = UVRect.bottom;
}
