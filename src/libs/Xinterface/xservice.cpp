...
long XSERVICE::BGFXGetTextureID(const char *sImageListName)
{
...
long XSERVICE::BGFXFindGroup(const char *sImageListName) const
{
    if (!sImageListName)
        return -1;
...
bool XSERVICE::BGFXGetTexturePos(long pictureNum, FXYRECT &texRect)
{
    if (pictureNum >= 0 && pictureNum < m_BGFXdwImageQuantity)
    {
        ...
        if (gn < m_BGFXdwListQuantity)
        {
            ...
            texRect.right =
                static_cast<float>(m_BGFXpImage[pictureNum].pTextureRect.right - m_fWAdd) / m_BGFXpList[gn].textureWidth;
            texRect.top =
                static_cast<float>(m_BGFXpImage[pictureNum].pTextureRect.top + m_fHAdd) / m_BGFXpList[gn].textureHeight;
            texRect.bottom =
                static_cast<float>(m_BGFXpImage[pictureNum].pTextureRect.bottom - m_fHAdd) / m_BGFXpList[gn].textureHeight;
            return true;
        }
    }
...
bool XSERVICE::BGFXGetTexturePos(long pictureNum, XYRECT &texRect)
{
    if (pictureNum >= 0 && pictureNum < m_BGFXdwImageQuantity)
    {
...
bool XSERVICE::BGFXGetTexturePos(int nTextureModify, long pictureNum, FXYRECT &texRect)
{
    FXYRECT rectTmp;
...
void XSERVICE::BGFXGetTextureCutForSize(const char *pcImageListName, const FXYPOINT &pntLeftTopUV, const XYPOINT &pntSize,
...
long XSERVICE::BGFXGetImageNum(const char *sImageListName, const char *sImageName)
{
    long retVal = -1;
            ...
            for (int i = 0; i < m_BGFXdwListQuantity; i++)
                if (!_stricmp(m_BGFXpList[i].sImageListName, sImageListName))
                {
                    for (int j = m_BGFXpList[i].pictureStart;
                         j < m_BGFXpList[i].pictureStart + m_BGFXpList[i].pictureQuantity;
                         j++)
